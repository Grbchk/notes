...
//
//---(event.currentTarget)---
//по-умолчанию события регистрируются при фазе всплытия:
<div onClick={handleDiv}>  {/* этот обработчик сработает вторым (на фазе всплытия, сначала отработает обработчик кнопки) */}
  <button onClick={handleButton}>кнопка</button>  {/*обработчик при клике по кнопке сработает первым (проверить: вывести в обработчиках сообщения console.log) */}
</div> </>
//регистрация событий на фазе захвата
<div onClickCapture={handleDiv}>   {/* добавим слово Capture к названию нужного события и оно будет зарегестрировано на фазе захвата */}
  <button onClick={handleButton}>кнопка</button>  {/* при клике по кнопке сначала сработает обработчик handleDiv */}
</div> </>
//Стандартное поведение всплытия и захвата способно приводить к
//коллизиям: в элемент с обработчиком событий может быть вложен
//другой элемент с таким же обработчиком.
//stopPropagation()
// - останавливает процессы всплытия на элементе
function handleButton(e) {
  e.stopPropagation();     //остановит всплытие на элементе button, обработчик anotherHandleDiv не зарегестрируется
}  </>
<div onClick={anotherHandleDiv} onClickCapture={handleDiv}>   {/* на обработчик anotherHandleDiv не будет наложен слушатель на фазе всплытия (он вообще не отработает)*/}
  <button onClick={handleButton}>кнопка</button>  {/* при клике по кнопке все-равно сначала сработает обработчик handleDiv из-за регистрации на фазе захвата */}
</div>
//Event.target
function Button() {
  function handle(e) {
  ...
  return (
    <div onClick={handle}>     {/* при клике на div значение e.target будет равно div */}
      <button>               {/* при клике на button значение e.target будет равно button */}
        <span></span>      {/* при клике на span e.target будет равен span */}
      </button>
    </div>
  );
}
//currentTarget
// - наоборот всегда указывает на элемент с добавленным слушателем событий
function handle(e) {
  console.log(e.currentTarget);
}
<div onClick={handle}>     {/* добавляем слушатель */}
  <button>
    <span></span>  {/* даже при клике по button или span значение e.currentTarget будет равно div */}
  </button>
</div>
//-----
//
//---(React.PureComponent)---
//создание чистого классового компонента (т.е. не вызывающего перерисовку всех дочерних компонентов при изменении родительского компонента в виртуальном DOM)
class Chat extends React.PureComponent {  //достаточно наследоваться не от React.Component, а от React.PureComponent
  render() {
    return (
      <div className="chat">я чат!</div>
    );
  };
}
//не подходит при сложных пропсах, для кторых требуется глубокое сравнение - shouldComponentUpdate()
//---
//---(React.memo)---
//создание чистого функционального компонента (т.е. не вызывающего перерисовку всех дочерних компонентов при изменении родительского компонента в виртуальном DOM)
const Chat = React.memo((props) => {  //достаточно обернуть компонент в функцию React.memo
  return (
    <div className="chat">я чат!</div>
  );
});
//так цепная реакция перерисовки дочерник компонентов будет обрываться
//на компоненте Chat, так как его (!)пропсы не меняются
//---
//---(Поверхностное сравнение)---
//React.memo и React.PureComponent используют алгоритм поверхностного сравнения
//предыдущее значение каждого пропса сравнивается с новым значением при помощи оператора ===
//пример, когда такое сравнение не сработает (по сути с любым сложным типом данных):
<MyPureComponent
...
class ThemeToggle extends React.Component {
  state = { theme: "светлая" };
  toggleTheme = () => {
    this.setState({theme: this.state.theme === "светлая" ? "тёмная" : "светлая",});
  };
  render() {
    return (
...
function ThemeToggle() {
  const [theme, setTheme] = React.useState("светлая");  //объявление двух переменных с помощью деструктурирующего присваивания из возвращаемого массива
  const toggleTheme = () => {setTheme(theme === "светлая" ? "тёмная" : "светлая");};
  return (
...
// [theme, setTheme] - результат хука (массив из двух элементов: theme - текущее значение, а setTheme функция-сеттер для измеения этого значения)
// React.useState - название хука
// ("светлая") - исходное значение при первом вызове
//---
//начальные значения могут быть почти любыми:
const [isBlocked, setIsBlocked] = React.useState(false); //true или false
const [user, setUser] = React.useState({ id: 1, name: 'Полина' }); //объект
//---
// (!) необходим неизменный порядок хуков между вызовами компонента
// - хуки не могут находиться внутри условных блоков, циклов и т.п.
// - хуки нельзя использовать в других функциях, обработчиках или модулях
// + можно вызывать их только из основной функции компонента
//---
//правила мутаций сложных объектов:
//пример 1:
  const [array, setArray] = React.useState(['Раз', 'Два', 'Три']);
  //неправильно:
  //   array.push('Четыре');
  //   setArray(array);
  //правильно с использованием оператора spread:
...
//React сравнивает старое и новое значение с помощью оператора ===, а значит, мы не можем передавать исходный объект, иначе проверка всегда будет возвращать true
//-----
//
//---(React.useEffect)---
//пример:
//аналогичный классовый компонент:
...
//функциональный компонент:
//первый аргумент хука — функция, код которой выполняется при монтировании компонента, изменении его пропсов или состояния
//второй аргумент хука называется dependencies (deps, или зависимости)
function NavBar = (props) => {
  const [state, setState] = useState({isVisible: true});
  useEffect(() => {     //первым аргументов передали функцию
    console.log('c:');
  }, [])      //передали пустой массив вторым аргументом в хук (из-за этого хук выполнится только при монтировании)
  return (
    <div></div>
  );
}
//(deps) - функциональный аналог классовых prevProps и prevState из componentDidUpdate
function NavBar = (props) => {
  const [state, setState] = useState({isVisible: true});
  useEffect(() => {
    console.log('c:');
  }, [state.isVisible])   //хук выполнится при монтировании и при изменении этого аргумента (этой зависисмости)
  return (
    <div></div>
  );
}
//в (deps) лучше передавать примитивы и только нужные ключи объекта,
//а для массивов использовать деструктуризацию, или JSON.stringify()
//обычные массивы и объекты замедляют работу кода, т.к. (deps) в хуке сравниваются с предыдущими значениями, и чем меньше в зависимостях будет сложных и вложенных объектов, тем быстрее будет работать код
useEffect(() => {
}, [
  ...props.products,
  someObject.nestedValue,
  JSON.stringify(filters.values)
])
//---
//---(асинхронные эффекты)---
//пример (страничка с информацией о фильмах):
//- - -
//файл index.js
...
//получаем данные с сервера и запускаем отрисовку странички
export default function App() {
  ...
  React.useEffect(() => getFilms(), []);   //данные запрашиваем один раз при монтировании
...
//---
//---(useEffect со сбросом)---
//аналог классового componentWillUnmount()
React.useEffect(()=>{
  //код эффекта, подписка событий
  return () => { //возвращаем код сброса
    //отписка от событий, закрытие соединений
  }
}, [])
//Эффект ведёт себя как совокупность двух методов жизненного цикла:
//componentDidMount и componentWillUnmount, но есть одно исключение.
//Если вовсе не передать второй аргумент в эффект, то сброс будет
//выполняться не только при размонтировании, но и при каждом рендере
//компонента. Также сброс будет вызываться при изменении зависимостей,
//переданных в хук useEffect вторым аргументом.
//пример:
import { useEffect } from 'react';
const Chat = ({chatId}) => {  //chatId - нужен для монтирования разных чатов с разной историей сообщений
  useEffect(() => {
    ChatAPI.getHistory(chatId); //получаем историю сообщений
    ChatAPI.subscribe(chatId); //подписываемся на получение новых сообщений
    return () => {  //отписываемся от получения новых сообщений
      ChatAPI.unsubscribe();  //если изменился chatId или при размонтировании
    }
  }, [chatId])
  return ()
}
...
function GameResult({points}) {
  ...
  const prevPoints = prevPointsRef.current;  //изменение current не ведёт к повторному рендерингу
  return (
    <h1>
      Новый рекорд: {points}, Предыдущий рекорд: {prevPoints}  {/* при первом рендере будет 0 и undefined */}
    </h1>
  );
}
const Game = () => {
...
//шаги рендера:
//1) Рендерим компонент Game с помощью ReactDOM.render.
//2) В компоненте Game инициализируется state и функция addPoint.
//3) Компонент Game рендерит кнопку и компонент GameResult со значением из state в качестве пропса points.
//4) Компонент GameResult инициализирует константу prevPointsRef с помощью хука useRef.
//   Начальное значение prevPointsRef.current равно undefined.
//5) Компонент GameResult рендерит такую разметку: <h1>Новый рекорд: 0, Предыдущий рекорд:</h1>.
//   Значение предыдущего рекорда берётся из prevPointsRef.current и всё ещё равно undefined,
//   поэтому в разметке нет значения предыдущего рекорда на данный момент.
//6) В компоненте GameResult после рендера вызывается useEffect, в нём мы присваиваем значение пропса points
//   рефу prevPointsRef.current. Помните, что изменение current не ведёт к повторному рендерингу.
//7) В компоненте Game нажимаем на кнопку и увеличиваем число баллов с нуля до единицы.
//8) Компонент Game рендерится повторно, так как изменился state, и обновлённое значение попадает в компонент GameResult.
//9) Компонент GameResult рендерит обновившуюся разметку, значение prevPointsRef.current не сбрасывается
//   на undefined, так как useRef уже был вызван при первом рендере.
//10) Компонент GameResult рендерит такую разметку: <h1>Новый рекорд: 1, Предыдущий рекорд: 0</h1>.
//    Теперь предыдущий рекорд отображается, так как был записан на шестом шаге.
//11) В компоненте GameResult после рендера вызывается useEffect, в нём мы снова присваиваем
//    значение пропса points нашему рефу prevPointsRef.current.
